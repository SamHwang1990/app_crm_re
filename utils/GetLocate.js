...
var getLocate = function(){
    var locate;
    return {
        getLocateFromPath: function(path){
        ...
        getLocateFromHeader: function(acceptLanguages){
            var langMatch = '',
                acceptLangLength,
                i = 0,
                currentLang;
        ...
        getLocate: function(path, acceptLanguages){
            locate = this.getLocateFromPath(path);
            if(!locate){
                locate = this.getLocateFromHeader(acceptLanguages);
            }
            if(!locate){
